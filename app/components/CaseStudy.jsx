...
const CASE_STUDIES = [
  ...
  },
  {
    ...
    description: "The most successful developments don't just build structures; they create ecosystems where work, play, and living coexist.",
    image: "/assets/blog/download (1).jpg",
...
export default function CaseStudySlider() {
  ...
  return (
    <section className="bg-white py-20 px-4">
      <div className="max-w-7xl mx-auto text-center mb-16">
        <p className="text-[#b79660] font-bold tracking-[0.3em] text-xs mb-4">CLIENT SUCCESS STORIES</p>
        <h2 className="text-4xl md:text-5xl font-bold mb-6">Real Challenges. <span className="text-[#b79660]">Real Impact.</span></h2>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          {CASE_STUDIES.map((item) => (
            <SwiperSlide key={item.id}>
              <div 
                onClick={() => handleDownload(item.pdfUrl, `CaseStudy-${item.id}.pdf`)}
              ...
              >
                ...
                <div className="p-8 flex flex-col flex-grow">
                  <div className="flex-grow">
                    <span className="text-gray-400 text-[10px] font-bold tracking-widest block mb-4 uppercase">{item.category}</span>
                    <h4 className="text-[#b79660] font-semibold text-lg mb-2">{item.title}</h4>
                    <h3 className="text-[#2d2d2d] font-bold text-xl mb-4 line-clamp-3">{item.subtitle}</h3>
